...
def issuer_card():
	...
	if not Cid:
		mb.showerror('Sudent ID cannot be zero!', 'Can\'t keep Student ID empty, it must have a value')
	else:
		return Cid
...
tree = ttk.Treeview(RB_frame, selectmode=BROWSE, columns=('Book Name', 'Book ID', 'Author', 'Status', 'Student Card ID'))
...
tree.heading('Student Card ID', text='Card ID of the Student', anchor=CENTER)
